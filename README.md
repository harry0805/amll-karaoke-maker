...
The default style is Nunito bold white text with a black 10% outline. Text size defaults to 5%, bottom margin to 0%, horizontal margin to 4% on each side, lyric area to 35%, background shade to 40%, and timing offset to 0 ms. The separate Duet color picker also defaults to white and applies to lines AMLL marks as duet. Background vocals follow their singer's color. The device font choices are Arial Rounded MT Bold, Arial, Impact, and Georgia, with installed fallbacks. Nunito is the default font. Loading a preset with an unavailable device font shows a notice and uses its fallback stack until the font is installed or changed. Nunito, Inter, Roboto Condensed, and Noto Serif are included as local assets in `public/fonts/`, with their licenses. Their CSS loads on selection, and html-to-image embeds the chosen font into rendered frames. A custom TTF, OTF, WOFF, or WOFF2 file can replace the single font stored in IndexedDB. Presets save the Custom selection but never the file. Without an uploaded font, Custom previews with the device font and blocks rendering. Set outline thickness to zero to turn it off. The outline scales with the text in both preview and export. An SVG filter expands the rendered line alpha after AMLL applies its word masks, avoiding clipped text strokes at word boundaries. Dimmed words still have a dimmer outline, and very thick outlines can soften fine letter details.
...
Settings are saved automatically to session storage and local storage. A tab restores its own session first; a new session falls back to the last settings saved on this device. Changing settings in one tab does not replace another tab's current settings. This includes timing offset.

The Settings step includes an immutable Default preset and named user presets. The preset dropdown applies saved settings. Manage presets opens a separate editor to save the current settings, rename or delete a preset, or replace its settings. Selecting a preset in the manager does not apply it. Each preset has its own Export action in Manage presets, which downloads that preset as versioned JSON. Import is also in the manager and accepts both individual presets and older collections. Import JSON validates the whole file before adding it, allows duplicate names and assigns fresh client-local IDs, and leaves current settings unchanged until a preset is selected. Exports contain names and settings only, never internal IDs, source media, or lyrics. The browser library retains IDs across reloads and tracks selection independently of names. Imports are limited to 1 MB and the library to 100 user presets.
