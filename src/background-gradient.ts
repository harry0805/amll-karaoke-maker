...
export function backgroundGradient(settings: Pick<Settings, 'backgroundColor' | 'shade' | 'shadeFadeStart'>): string {
  const rgb = [1, 3, 5].map(start => parseInt(settings.backgroundColor.slice(start, start + 2), 16)).join(' ');
  ...
  if (settings.shadeFadeStart === 100) return `linear-gradient(${color(opacity)}, ${color(opacity)})`;
