...
export interface StoredExport { id: string; name: string; createdAt: number; file: File }
...
async function removeFiles(dir: FileSystemDirectoryHandle, id: string) {
  for (const suffix of ['.json', '.mp4']) {
    try { await dir.removeEntry(id + suffix); }
...
export async function writeStoredExport(
  ...
  return navigator.locks.request(lockName(id), { signal }, async () => {
    ...
    try {
      ...
      const metadata = await (await dir.getFileHandle(id + '.json', { create: true })).createWritable();
      try { await metadata.write(JSON.stringify(record)); await metadata.close(); }
      catch (error) { await metadata.abort().catch(() => {}); throw error; }
      signal.throwIfAborted();
      return { ...record, file };
    } catch (error) {
  ...
  for (const id of ids) {
    await navigator.locks.request(lockName(id), { ifAvailable: true }, async lock => {
      ...
      try {
        ...
        if (error instanceof SyntaxError || (error instanceof DOMException && error.name === 'NotFoundError')) await removeFiles(dir, id);
        else throw error;
      }
    });
  }
