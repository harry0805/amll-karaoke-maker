...
async function decodeCustom(file: File) {
  if (!(file instanceof Blob) || typeof file.name !== 'string') throw new Error('The saved font is invalid. Upload it again.');
  ...
  try { await face.load(); } catch { throw new Error('This font could not be loaded. Choose a valid TTF, OTF, WOFF, or WOFF2 file.'); }
  const uri = await dataURL(new Blob([bytes]));
  return { file, face, css: `@font-face{font-family:"Karaoke Custom Font";font-style:normal;font-weight:100 900;src:url("${uri}")}` };
}
...
export function customFontName(): string | undefined { return custom?.file.name; }
...
export function requireCustomFont(font: FontKey) {
  if (font === 'custom' && !custom) throw new Error('Upload a custom font in Settings or choose another font before rendering.');
}
...
async function loadFontStyles(font: FontKey, family: string) {
  ...
  if (!pending) {
    pending = new Promise<void>((resolve, reject) => {
      ...
      link.rel = "stylesheet";
      link.href = new URL(`fonts/${family.replace(" Variable", "").toLowerCase().replaceAll(" ", "-")}/wght.css`, document.baseURI).href;
      link.onload = () => resolve();
      link.onerror = () => { link.remove(); reject(new Error("Could not load font styles. Try selecting the font again.")); };
...
export async function loadLyricFont(font: FontKey, text: string) {
  ...
  if (!faces.length) throw new Error(`Could not load ${family.replace(' Variable', '')}. Reload the page and try again.`);
}
...
export async function lyricFontCSS(font: FontKey): Promise<string> {
  if (font === 'custom') { requireCustomFont(font); return custom!.css; }
...
export function setStageFontCSS(stage: HTMLElement, css: string) { stageFontCSS.set(stage, css); }
export function getStageFontCSS(stage: HTMLElement): string { return stageFontCSS.get(stage) ?? ''; }
