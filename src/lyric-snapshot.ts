...
export async function snapshotLyrics(stage: HTMLElement, fullTree = false): Promise<HTMLImageElement> {
  ...
  if (!fullTree) {
    ...
    const margin = stage.clientHeight * .16;
    for (const group of stage.querySelectorAll<HTMLElement>('[class*="_lyricLineWrapper"]')) {
      ...
      for (const line of group.querySelectorAll('[class*="_lyricLine"]:not([class*="_lyricLineWrapper"])')) {
        const child = line.getBoundingClientRect();
        top = Math.min(top, child.top); bottom = Math.max(bottom, child.bottom);
      }
    ...
    for (const line of stage.querySelectorAll<HTMLElement>('[class*="_lyricMainLine"], [class*="_lyricSubLine"]')) {
      ...
      let leftPx = Infinity, rightPx = -Infinity, topPx = Infinity, bottomPx = -Infinity;
      while (walker.nextNode()) {
        if (!walker.currentNode.textContent?.trim()) continue;
        range.selectNodeContents(walker.currentNode);
        const text = range.getBoundingClientRect();
        leftPx = Math.min(leftPx, text.left); rightPx = Math.max(rightPx, text.right);
        topPx = Math.min(topPx, text.top); bottomPx = Math.max(bottomPx, text.bottom);
      }
      const padding = parseFloat(getComputedStyle(line).fontSize) * 1.2;
      const left = Math.max(-.2, (leftPx - box.left - padding) / box.width);
      const right = Math.min(1.2, (rightPx - box.left + padding) / box.width);
      const top = Math.max(-.5, (topPx - box.top - padding) / box.height);
      const bottom = Math.min(1.5, (bottomPx - box.top + padding) / box.height);
      if (right <= left || bottom <= top) continue;
      const key = String(outlineBounds.size);
      outlineBounds.set(key, { x: left * 100, y: top * 100, width: (right - left) * 100, height: (bottom - top) * 100 });
      line.dataset.snapshotOutline = key; marked.push(line);
    }
  }
  let uri: string;
  try {
