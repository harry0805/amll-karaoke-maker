...
export interface Settings {
  fontSize: number; bottom: number; height: number; offset: number; shade: number; resolution: number; fps: number;
  textColor: string; duetColor: string; font: keyof typeof fonts; outlineColor: string; outlineWidth: number;
}
export const defaults: Settings = {
  fontSize: 4.4, bottom: 4, height: 32, offset: 0, shade: 40, resolution: 1080, fps: 30,
  textColor: '#ffd45a', duetColor: '#7dd3fc', font: 'rounded', outlineColor: '#14141c', outlineWidth: 5,
};
export function validateSettings(input: unknown): Settings {
  ...
  const s = { textColor: defaults.textColor, duetColor: defaults.duetColor, font: defaults.font, outlineColor: defaults.outlineColor, outlineWidth: defaults.outlineWidth, ...input } as Settings;
  for (const [key, min, max] of [['fontSize', 2, 8], ['bottom', 0, 20], ['height', 20, 50], ['offset', -600000, 600000], ['shade', 0, 80], ['outlineWidth', 0, 12]] as const) {
  ...
  return { fontSize: s.fontSize, bottom: s.bottom, height: s.height, offset: s.offset, shade: s.shade, resolution: s.resolution, fps: s.fps, textColor: s.textColor, duetColor: s.duetColor, font: s.font, outlineColor: s.outlineColor, outlineWidth: s.outlineWidth };
}
