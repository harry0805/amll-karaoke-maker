...
function stops(fade: number, opacity = 80) {
  const gradient = backgroundGradient({ ...defaults, backgroundColor: '#123abc', shade: opacity, shadeFadeStart: fade });
  return [...gradient.matchAll(/rgb\(18 58 188 \/ ([\d.]+)\) ([\d.]+)%/g)].map(match => ({ alpha: Number(match[1]), position: Number(match[2]) }));
}

test('fade eases out of solid opacity and into transparency', () => {
  const samples = stops(50);
  expect(samples[0]).toEqual({ alpha: .8, position: 50 });
  expect(samples[8]).toEqual({ alpha: .675, position: 62.5 });
  expect(samples[16]).toEqual({ alpha: .4, position: 75 });
  expect(samples[24]).toEqual({ alpha: .125, position: 87.5 });
  expect(samples.at(-1)).toEqual({ alpha: 0, position: 100 });
  expect(samples[0]!.alpha - samples[1]!.alpha).toBeLessThan(.003);
...
test('fade limits keep their meaning and zero opacity stays transparent', () => {
  ...
  expect(backgroundGradient({ ...defaults, shade: 100, shadeFadeStart: 100 })).toBe('linear-gradient(rgb(0 0 0 / 1), rgb(0 0 0 / 1))');
});
