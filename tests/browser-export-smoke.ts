...
try {
  ...
  await page.route('**/test-config/browser-test/config', (route) =>
    route.fulfill({ json: { ttml, settings: defaults } }),
  );
  ...
  const result = await page.evaluate(
    async ({ sourceBytes, ttml, settings }) => {
      const canvas = document.createElement('canvas');
      const progress: number[] = [];
      const start = performance.now();
      const blob = await (window as any).exportVideo({
        video: new Blob([new Uint8Array(sourceBytes)]),
        ttml,
        settings,
        preview: canvas,
      ...
      return {
        bytes: Array.from(new Uint8Array(await blob.file.arrayBuffer())),
        frames: Math.max(...progress),
        seconds: (performance.now() - start) / 1000,
        leaked: document.querySelectorAll('.export-stage').length,
      };
    },
  ...
  await Bun.write(target, new Uint8Array(result.bytes));
  ...
  assert.equal(v.nb_frames, '48');
  assert.equal(v.avg_frame_rate, '24/1');
  assert.equal(v.codec_name, 'h264');
  assert.equal(a.codec_name, 'aac');
  assert(Math.abs(Number(info.format.duration) - 2) < 0.06);
  const audioPackets = async (path: string) =>
    JSON.parse(
      await command([
        'ffprobe',
        '-v',
      ...
      .packets.filter((p: any) => Number(p.pts_time) + Number(p.duration_time) > 0)
      .map((p: any) => ({ hash: p.data_hash, time: Number(p.pts_time) }));
  assert.deepEqual(
    await audioPackets(source),
    await audioPackets(target),
    'Presented AAC packets and timestamps should be copied unchanged',
  );
  const raw = Bun.spawn(
    [
      'ffmpeg',
  ...
  for (let i = 0; i < pixels.length; i += 3) {
    ...
    if (r > 150 && g > 100 && b < 130) gold++;
    if (r > 120 && g < 90 && b > 50) pink++;
  }
  assert(gold > 50, `Expected lyric fill, got ${gold} pixels`);
  assert(pink > 20, `Expected outline, got ${pink} pixels`);
  await command([
    'ffmpeg',
    '-v',
    'error',
  ...
  console.log(
    `PASS: local H.264/AAC export, 48 frames, timing, colored lyrics/outline, cleanup, zero network requests. ${result.seconds.toFixed(1)}s. ${dir}`,
  );
  const cancelled = await page.evaluate(
    async ({ sourceBytes, ttml, settings }) => {
      ...
      try {
        ...
        return controller.signal.aborted && !document.querySelector('.export-stage');
      }
    },
  ...
  await command([
    ...
    '-an',
    '-vf',
    "select='if(lt(t,1),1,not(mod(n,2)))'",
    '-fps_mode',
    'vfr',
    '-c:v',
    'libx264',
  ...
  const silentResult = await page.evaluate(
    async ({ sourceBytes, ttml, settings }) => {
      const blob = await (window as any).exportVideo({
        video: new Blob([new Uint8Array(sourceBytes)]),
        ttml,
  ...
  const silentTarget = join(dir, 'silent-result.mp4');
  await Bun.write(silentTarget, new Uint8Array(silentResult));
  const timestamps = async (path: string) =>
    JSON.parse(
      await command([
        'ffprobe',
        '-v',
    ...
    ).frames.map((f: any) => Number(f.pts_time));
  const before = await timestamps(silent),
    after = await timestamps(silentTarget);
  assert.equal(before.length, after.length);
  before.forEach((t: number, i: number) =>
    assert(Math.abs(t - after[i]) < 0.00001, 'Preserve variable frame timestamps'),
  );
  const silentInfo = JSON.parse(
    await command(['ffprobe', '-v', 'error', '-show_streams', '-of', 'json', silentTarget]),
  );
  assert.equal(silentInfo.streams.length, 1);
  assert.deepEqual(apiRequests, []);
  console.log(
    'PASS: cancellation, subsequent export, silent input, original VFR frame timestamps, unchanged AAC packets',
  );
  ...
  const unsupported = await page.evaluate(
    async ({ bytes, ttml, settings }) => {
      try {
        await (window as any).exportVideo({
          video: new Blob([new Uint8Array(bytes)]),
          ttml,
          settings,
          preview: document.createElement('canvas'),
          signal: new AbortController().signal,
    ...
    {
      bytes: Array.from(new Uint8Array(await Bun.file(opus).arrayBuffer())),
      ttml,
      settings: defaults,
    },
  );
  ...
  const stored = await page.evaluate(async () => {
    ...
    ).getDirectoryHandle('karaoke-exports-v1');
    const names: string[] = [];
    for await (const name of dir.keys()) names.push(name);
    return {
      count: list.length,
      fileBacked: list.every((item: any) => item.file instanceof File),
      files: names.length,
    };
  });
  assert.deepEqual(
    stored,
    { count: 2, fileBacked: true, files: 4 },
    'Only the two successful MP4s and their records should remain',
  );
