...
assert.equal(await p.exited, 0, error);
const browser = await chromium.launch();
try {
  const page = await browser.newPage({ viewport: { width: 1920, height: 1080 } });
  const ttml = await benchmarkLyrics(),
    settings = { ...defaults, offset: -40000 };
  ...
  const result = await page.evaluate(
    async ({ bytes, ttml, settings }) => {
      ...
      let frames = 0;
      const result = await (window as any).exportVideo({
        video: new Blob([new Uint8Array(bytes)]),
        ttml,
        settings,
        preview: document.createElement('canvas'),
        signal: new AbortController().signal,
        onProgress: (p: any) => (frames = p.frames),
      });
      const seconds = (performance.now() - start) / 1000;
      const size = result.file.size;
      await (window as any).deleteStoredExport(result.id);
