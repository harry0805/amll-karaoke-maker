...
try {
  ...
  // A bright lyric must exist in the lower portion of the composed output.
  ...
  await raw.exited;
  let bright = 0;
  for (let i = 640 * 200 * 3; i < pixels.length; i += 3) if (pixels[i]! > 180 && pixels[i + 1]! > 180 && pixels[i + 2]! > 180) bright++;
