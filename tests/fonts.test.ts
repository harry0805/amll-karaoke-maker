...
test('Nunito is the default and every font choice is valid', () => {
  ...
  expect(fonts.rounded).toBe('"Arial Rounded MT Bold", "SF Pro Rounded", Arial, "PingFang SC", sans-serif');
  for (const font of Object.keys(fonts) as Array<keyof typeof fonts>) expect(validateSettings({ ...defaults, font }).font).toBe(font);
...
test('upload validation rejects mislabeled files and allows supported font containers', () => {
  for (const [name, signature] of [['a.ttf',0x00010000],['a.otf',0x4f54544f],['a.woff',0x774f4646],['a.WOFF2',0x774f4632]] as const) {
    const bytes = new ArrayBuffer(4); new DataView(bytes).setUint32(0, signature);
...
test('custom selection round-trips without font bytes or filenames in presets', () => {
  const file = serializePresets([{ name: 'Custom', settings: { ...defaults, font: 'custom', fontFile: 'private-font.ttf', fontBytes: 'secret-data' } as typeof defaults }]);
