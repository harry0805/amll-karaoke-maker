...
try {
  ...
  async function dotsAt(time: number) {
    ...
    return page.evaluate(() => {
      ...
      return { opacity: getComputedStyle(el).opacity, dots: Array.from(el.children).map(el => Number(getComputedStyle(el).opacity)), top: rect.top - box.top, bottom: rect.bottom - box.top, height: box.height, filter: getComputedStyle(el).filter, transform: el.style.transform };
    });
  }
  await load();
  for (const time of [8000, 20000, 27000, 18000, 6000]) {
    ...
    assert(dots.dots.some(opacity => opacity > 0.2), JSON.stringify(dots));
  ...
  assert.deepEqual(played.dots, seek.dots, 'Dots should reflect the same media time during playback and seeking');
  ...
  console.log('PASS: outlined dots, outline off, forward/backward interlude seeks, viewport placement, and playback parity');
