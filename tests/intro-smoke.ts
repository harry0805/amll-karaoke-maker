...
try {
  ...
  const ttml =
    '<tt xmlns="http://www.w3.org/ns/ttml"><body><div><p begin="12s" end="14s"><span begin="12s" end="14s">First line</span></p></div></body></tt>';
  await page.route('**/test-config/intro/config', (route) =>
  ...
  assert.equal(
    await page.locator('[class*="_interludeDots"]').evaluate((el) => getComputedStyle(el).opacity),
    '1',
    'Prelude dots remain visible before the first sung line',
  );
