...
try {
  ...
  assert(
    compact.filter(
      (row) => row.visible === 'visible' && row.y >= 0 && row.y + row.height < row.boxHeight,
    ).length > 3,
    'More than three lines should fill the available space',
  );
  ...
  assert(
    spacious[1]!.y - spacious[0]!.y > (compact[1]!.y - compact[0]!.y) * 1.5,
    'Line spacing must affect measured row positions',
  );
  console.log(
    'PASS: more than three visible lines, offscreen upcoming lines, scrolling entry, and line spacing',
  );
} finally {
  await browser.close();
}
