...
try {
  ...
  const ttml = process.env.TTML_PATH
    ? await Bun.file(process.env.TTML_PATH).text()
    : `<tt xmlns="http://www.w3.org/ns/ttml"><body><div><p begin="0s" end="4s"><span begin="0s" end="1s">Out</span><span begin="1s" end="2s">lined </span><span begin="2s" end="4s">letters</span></p></div></body></tt>`;
  const settings = {
    ...defaults,
    textColor: '#ffffff',
    fontSize: 4,
  ...
  await page.route('**/test-config/outline-test/config', (route) =>
  ...
  await page.evaluate(() => {
    document.body.style.setProperty('background', '#8d5265', 'important');
  });
  ...
  for (const time of times) {
    ...
    await page.screenshot({
      path: `/tmp/karaoke-outline-${process.env.CAPTURE_LABEL || 'after'}-${time}.png`,
    });
  }
