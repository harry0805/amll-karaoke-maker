...
try {
  ...
  await page.route('**/test-config/overlap-test/config', (route) =>
    route.fulfill({ json: { ttml, settings: { ...defaults, height: 20 } } }),
  );
  ...
  async function position(text: string) {
    return page.evaluate((text) => {
      const main = Array.from(document.querySelectorAll('[class*="_lyricMainLine"]')).find(
      ...
      return {
        ...
        visible: getComputedStyle(main).visibility,
      };
    }, text);
  }
  await page.evaluate(async () => {
    for (let t = 0; t < 1800; t += 1000 / 30) await window.renderFrame(t, 1000 / 30);
  });
  const first = await position('Short first line');
  const secondBefore = await position('Long second line');
  await page.evaluate(async () => {
    for (let t = 1800; t <= 3100; t += 1000 / 30) await window.renderFrame(t, 1000 / 30);
  });
  ...
  assert(
    Math.abs(secondAfter.y - first.y) < 5,
    'Remaining active line should take the first line position',
  );
  assert(secondAfter.y < secondAfter.boxHeight * 0.25, 'Active line should be near the top');
  await page.evaluate(async () => {
    for (let t = 3100; t <= 6100; t += 1000 / 30) await window.renderFrame(t, 1000 / 30);
  });
  ...
  console.log(
    'PASS: overlapping lines advance individually, top alignment, next line, and backward seek',
  );
} finally {
  await browser.close();
}
