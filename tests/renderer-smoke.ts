...
try {
  ...
  await page.route('**/test-config/style-test/config', (route) =>
  ...
  async function visibleAt(time: number) {
    ...
    return page.evaluate(() =>
      Array.from(document.querySelectorAll<HTMLElement>('[class*="_lyricMainLine"]'))
  ...
  const colors = await page.evaluate(() =>
    Array.from(document.querySelectorAll('[class*="_lyricMainLine"]')).map(
      (el) => getComputedStyle(el).color,
    ),
  );
  assert.deepEqual(
    colors,
    ['rgb(255, 212, 90)', 'rgb(255, 212, 90)', 'rgb(125, 211, 252)'],
    'Only duet lines should use the duet color',
  );
  const duetOutline = await page.evaluate(() => {
    ...
    return {
      ...
      color: filter.querySelector('feFlood')!.getAttribute('flood-color'),
      radius: Number(filter.querySelector('feMorphology')!.getAttribute('radius')),
    };
  });
  assert(duetOutline.filter.includes('-duet'));
  assert.equal(duetOutline.color, '#cc00ff');
  assert(Math.abs(duetOutline.radius - 0.4752) < 0.01);
  ...
  assert(
    (await visibleAt(1000)).includes('Golden words'),
    'Seeking backward must restore the line',
  );
  ...
  for (const font of Object.keys(fonts) as (keyof typeof fonts)[]) {
    ...
    assert(
      style.font.includes(fonts[font].split(',')[0]!.replaceAll('"', '')),
  ...
  for (const horizontalMargin of [0, 15, 30]) {
    ...
    const box = await page.evaluate(() => {
      ...
      return {
        left: (lyrics.left - stage.left) / stage.width,
        right: (stage.right - lyrics.right) / stage.width,
      };
    });
  ...
  assert.equal(
    await page.evaluate(
      () => getComputedStyle(document.querySelector('[class*="_lyricDuetLine"]')!).color,
    ),
    'rgb(255, 68, 170)',
  );
  console.log(
    'PASS: exact end cutoffs, independent background vocals, offset, backward seeking, fonts, colors, and outline off',
  );
} finally {
  await browser.close();
}
