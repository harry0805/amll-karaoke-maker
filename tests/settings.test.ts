...
test('rejects invalid or unsupported export parameters', () => {
  ...
  for (const change of [{ offset: NaN }, { shade: 100 }, { fontSize: '4' }, { bottom: -1 }, { font: 'unknown' }, { font: 'toString' }, { textColor: 'url(example)' }, { outlineColor: null }, { outlineWidth: 13 }]) expect(() => validateSettings({ ...defaults, ...change })).toThrow();
