...
test('rejects invalid or unsupported export parameters', () => {
  ...
  for (const change of [{ fps: 120 }, { resolution: 0 }, { offset: NaN }, { shade: 100 }, { fontSize: '4' }, { bottom: -1 }, { font: 'unknown' }, { font: 'toString' }, { textColor: 'url(example)' }, { outlineColor: null }, { outlineWidth: 13 }]) expect(() => validateSettings({ ...defaults, ...change })).toThrow();
...
test('accepts an outline-free style and upgrades older export settings', () => {
  ...
  const { textColor, duetColor, font, outlineColor, outlineWidth, ...old } = defaults;
