...
export async function benchmarkLyrics() {
 ...
 const lines=Array.from({length:90},(_,i)=>{
  const start=i*4, end=start+3;
  const spans=words.map((word,j)=>`<span begin="${start+j*.25}s" end="${j===words.length-1?end:start+(j+1)*.25}s">${word}${j<words.length-1?' ':''}</span>`).join('');
  const bg=i%7===0?`<span ttm:role="x-bg" begin="${start+1}s" end="${end+1}s"><span begin="${start+1}s" end="${end+1}s">Echoes by the waves</span></span>`:'';
